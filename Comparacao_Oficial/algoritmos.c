...
{
    return (float)item.lucro / (item.peso + item.volume);
}
...
int resolverDP(Item itens[], int num_itens, int W_max, int V_max)
{
    ...
    if (dp == NULL)
    {
        printf("Erro: Faltou memoria RAM para a matriz DP!\n");
        return -1;
    }
...
static void prepararItensGRASP(Item itens[], int n, int maxPeso, int maxVolume)
{
    for (int i = 0; i < n; i++)
    {
        //Reaproveitar o esmo score do guloso otimizado 
        itens[i].score = calcularScore(itens[i], maxPeso, maxVolume);
        itens[i].selecionado = 0;
    }
}
                                 ...
                                 float alphaGRASP, int *pesoFinal, int *volumeFinal)
{
    ...
    while(1)
    {
        ...
        //Primeiro passo: localizar apenas candidatos viáveis 
        for (int i = 0; i < n; i++)
        {
        ...
        //Escolha aleatória dentro da lista restrita
                    ...
                    int *pesoFinal, int *volumeFinal)
{
    ...
    for(int interacao = 0; interacao < interacoes; interacao++)
    {
        ...
        //Gera uma solução inicial viável e diversificada
